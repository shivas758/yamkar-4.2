...
import { initConnectionManager, cleanupConnectionManager, ConnectionStatus } from "@/lib/connectionManager";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const statusListener = eventBus.subscribe(EVENTS.CONNECTION_STATUS_CHANGED, ({ status }) => {
      ...
      // If disconnected or error, we need to update UI accordingly
      if (status === ConnectionStatus.DISCONNECTED || status === ConnectionStatus.ERROR) {
        // If there's no user and we're disconnected, we can just redirect to login
        if (!user) {
          router.push('/');
        }
      }
    ...
    // Listen for data refresh events to update user data if needed
    const refreshListener = eventBus.subscribe(EVENTS.DATA_REFRESH_NEEDED, async () => {
      console.log('Connection restored, refreshing user data if needed');
      if (user) {
        try {
    ...
    return () => {
      // Cleanup the connection manager and event listeners
      statusListener();
      refreshListener();
  ...
  useEffect(() => {
    // The connection manager now handles visibility changes directly,
    // so we don't need our own handler here
    
    // We'll keep track of our own last active time for other purposes
