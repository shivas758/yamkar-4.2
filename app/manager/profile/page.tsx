...
import { User, Mail, Phone, ShieldCheck, Calendar, Camera, Loader2, X, Users, Building } from "lucide-react"
    ...
    return (
      <div className="flex items-center justify-center min-h-[70vh]">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-[#6B8E23]">Loading profile...</p>
        </div>
      </div>
    );
  }
    ...
    <>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <h1 className="text-2xl font-bold text-[#6B8E23]">My Profile</h1>
          {!isEditing ? (
            <Button onClick={() => setIsEditing(true)} className="bg-[#6B8E23] hover:bg-[#5a7a1e]">
              Edit Profile
            </Button>
          ) : (
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setIsEditing(false)}>Cancel</Button>
              <Button onClick={handleSave} disabled={isLoading} className="bg-[#6B8E23] hover:bg-[#5a7a1e]">
                {isLoading ? (
                  <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-1">
            <CardContent className="pt-6 flex flex-col items-center">
              <div className="relative">
                ...
                >
                  {avatarUrl ? (
                    ...
                    <AvatarFallback className="bg-[#6B8E23] text-white text-2xl">
              ...
              <h2 className="text-xl font-semibold text-[#6B8E23]">{profileData.name}</h2>
              <Badge className="mt-1 bg-[#6B8E23]">{profileData.role}</Badge>
              ...
              <div className="w-full mt-6 space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-sm">Team size: {profileData.employeeCount} employees</span>
                </div>
                
                <div className="flex items-center">
                  <Calendar className="h-5 w-5 mr-2 text-[#6B8E23]" />
                  <span className="text-sm">Joined: {profileData.joinDate || "N/A"}</span>
                </div>
              </div>
              ...
              >
                <Camera className="mr-2 h-4 w-4 text-[#6B8E23]" />
                <span className="text-[#6B8E23] font-medium">Change Profile Picture</span>
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="p-2 border rounded-md bg-gray-50">{profileData.employeeCount} employees</div>
                <p className="text-xs text-muted-foreground">Team assignment is managed by administrators</p>
              </div>
            </CardContent>
          </Card>
        </div>
